...
<template>
  <aside
    class="absolute left-0 top-0 z-[9999] flex mt-1 h-screen w-72.5 flex-col overflow-y-hidden  bg-[#ffff] text-gray-800 shadow-2xl  duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0"
    :class="{
  ...
  >
    <!-- SIDEBAR HEADER -->
    <div class="flex items-center justify-between gap-2 px-6 py-5.5 lg:py-6.5 p-4">
      <Link href="">
        <img src="/images/code.jpeg" alt="Logo" class="w-[80px] rounded-sm"/>
      </Link>
          ...
          <div>
            ...
            <ul class="mb-6 flex flex-col gap-1.5">
                  <div v-for="(menuItem, index) in menuGroup.menuItems" 
                      :key="index">
                    <SidebarItem  v-if="$page.props?.auth?.roles[0] == 'admin'"
                      :item="menuItem"
                      :index="index"
                      :current_route="menuItem.route"
                      class="border-b hover:text-gray-500"
                    />
                    <SidebarItem  v-if="$page.props?.auth?.roles[0] == 'user' && menuItem.route != 'users.list'"
                      :item="menuItem"
                      :index="index"
                      :current_route="menuItem.route"
                      class="border-b hover:text-gray-500"
