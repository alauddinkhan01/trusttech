...
<template>
  ...
  <div v-for="(item, index) in cardItems"
    :key="index" class="rounded-lg border border-stroke bg-white py-3 px-7.5 shadow-default dark:border-strokedark dark:bg-boxdark p-4 shadow-sm" :class="$page.props.auth.roles[0] == 'user' && item.total == 'Total Posts' ? '' : 'hidden'" >
<div v-if="$page.props.auth.roles[0] == 'user' && item.total == 'Total Posts'"
  >
    <div
      class="flex h-11.5 w-11.5 items-center justify-self-end rounded-full bg-meta-2 dark:bg-meta-4"
      v-html="item.icon"
    ></div>
...
<div v-if="$page.props.auth.roles[0] == 'admin'"
    class="rounded-lg border border-stroke bg-white py-3 px-7.5 shadow-default dark:border-strokedark dark:bg-boxdark p-4 shadow-sm"
  >
    <div
      class="flex h-11.5 w-11.5 items-center justify-self-end rounded-full bg-meta-2 dark:bg-meta-4"
      v-html="item.icon"
